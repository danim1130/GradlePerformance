...
class Torrent(val data: TorrentData){
    ...
    private val peersRequestedBlocks = mutableMapOf<RequestBlock, MutableSet<Peer>>()
    ...
    private fun peerBlockRequest(peer: Peer, block: RequestBlock){
        launch(CommonPool){
            val loadBlock = synchronized(peersRequestedBlocks){
                if (peersRequestedBlocks.containsKey(block)){
                    peersRequestedBlocks[block]!!.add(peer)
                    false
                } else {
                    peersRequestedBlocks[block] = mutableSetOf(peer)
                    true
                }
            }
            if (loadBlock) {
                val data = pieceMapper.readBlock(block.pieceIndex * data.info.pieceLength + block.baseOffset, block.length)
                val piece = PieceBlock(block.pieceIndex, block.baseOffset, data)
                val peerSet = synchronized(peersRequestedBlocks){ peersRequestedBlocks.remove(block) }
                peerSet?.forEach{it.sendPiece(piece); uploaded += block.length}
            }
        }
    }
    ...
    private fun peerCancelMessage(peer: Peer, block: RequestBlock) {
        launch {
            synchronized(peersRequestedBlocks){
                peersRequestedBlocks[block]?.remove(peer)
            }
        }
    }
